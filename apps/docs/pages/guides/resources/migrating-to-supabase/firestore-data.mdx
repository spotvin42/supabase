...
### Insert Point type from lat and long, and converting it in another document
```js
module.exports = async (collectionName, doc, recordCounters, writeRecord) => {
  const users_location = {
    geohash: null,
    geopoint: `(${doc.lat}, ${doc.lng})`,
    city: doc.city,
    user_id: doc.firestore_id,
  };
  
  await writeRecord("users_location", users_location, recordCounters);
